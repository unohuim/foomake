...
export function mount(rootEl, payload) {
    ...
    Alpine.data('salesProductsIndex', () => ({
        ...
        connectUrlBase: safePayload.connectUrlBase || '',
        ...
        async connectSelectedSource() {
            this.connectError = '';
            this.errors = emptyErrors();

            const response = await fetch(`${this.connectUrlBase}/${this.selectedSource}/connect`, {
                method: 'POST',
                headers: {
            ...
            if (response.status === 422) {
                ...
                this.connectError = data.message || 'Unable to connect source.';
                return;
            }

            if (!response.ok) {
                this.connectError = 'Unable to connect source.';
                return;
            }

            this.sources = this.sources.map((source) => source.value === this.selectedSource
                ? { ...source, connected: true }
                : source);
            this.showToast('success', 'Source connected.');
        },
        async loadPreview() {
            this.previewError = '';
            this.importError = '';
