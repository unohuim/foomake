...
<div x-data="{{ $workflowExpression }}" x-show="hasActions()" x-cloak data-workflow-action-button>
    <x-dropdown align="right" width="w-72" contentClasses="rounded-xl border border-slate-200 bg-white p-1.5 shadow-lg ring-1 ring-black/5">
        <x-slot name="trigger">
            ...
            >
                <span class="inline-flex items-center gap-2 px-3 py-2 text-sm font-medium text-slate-700">
                    ...
                    <span class="text-sm font-semibold uppercase text-slate-700" x-text="workflow.status"></span>
                </span>
        ...
        <div class="space-y-1" role="menu">
            <template x-for="action in workflow.actions" :key="action.type">
                <button
                    type="button"
